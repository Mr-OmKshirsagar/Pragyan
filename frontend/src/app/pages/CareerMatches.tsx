...
const defaultCareersData: Record<string, Partial<Career>> = {
  'Software Engineer': {
  ...
  'AI Engineer': {
    tags: ['AI/ML', 'Cutting Edge', 'High Demand'],
    salary: '$100k - $180k',
    growth: '+38%',
    icon: '🤖',
  },
  ...
  'Cyber Security Engineer': {
    tags: ['Security', 'Protection', 'In-Demand'],
    salary: '$90k - $150k',
    growth: '+33%',
    icon: '🔒',
  },
  ...
  'Ethical Hacker': {
    tags: ['Security', 'Analysis', 'High Value'],
    salary: '$85k - $145k',
    growth: '+33%',
    icon: '🕵️',
  }
...
export function CareerMatches({ onSelectCareer, onBack, onProfile }: CareerMatchesProps) {
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      try {
        setIsLoading(true);
        const response = await apiFetch<{ data: Array<{ career: string; score: number; reason: string }> }>('/api/recommendations/careers', undefined, { auth: true });
        
        if (response.data && response.data.length > 0) {
          const mappedCareers = response.data.map((item, index) => {
            const defaults = defaultCareersData[item.career] || {
              tags: ['Tech', 'Growth'],
              salary: '$70k - $130k',
              growth: '+15%',
              icon: '🚀',
            ...
            return {
              id: item.career.toLowerCase().replace(/\s+/g, '-'),
              title: item.career,
              match: Math.max(50, item.score || 80 - (index * 5)),
              reason: item.reason,
              ...defaults,
            } as Career;
          });
          ...
          // Fallback if empty array or no data
          setFallbackData();
        }
      } catch (error) {
        console.error('Failed to fetch career matches:', error);
        setFallbackData();
      } finally {
        setIsLoading(false);
      }
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <GlassCard strong className="mt-8 p-6 text-center">
          ...
          <GlassButton variant="secondary" onClick={onProfile}>Complete Your Profile</GlassButton>
        </GlassCard>
      </div>
    </div>
  );
